...
1. `ENABLE_OAUTH_GROUP_ID_RESOLUTION` - If `true` (and `ENABLE_OAUTH_GROUP_MANAGEMENT` is also `true`), Open WebUI will automatically resolve UUID-shaped group IDs to their display names using the identity provider's API at login time. This is useful when your identity provider (e.g., Microsoft Entra ID) returns group GUIDs instead of human-readable names — which is the default behavior when the groups claim is configured to use "Group ID" in Azure. Currently supported for the Microsoft provider via the Graph API. No additional Azure permissions or scopes are required beyond the defaults. IDs that cannot be resolved are left unchanged. Defaults to `false`.
