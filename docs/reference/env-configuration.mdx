...
- Type: `bool`
- Default: `False`
- Description: When enabled, UUID-shaped group claim values (e.g., from Microsoft Entra ID configured with "Group ID" as the groups claim source) are automatically resolved to human-readable display names using the identity provider's API before group matching occurs. Currently supports Microsoft (via the Graph API `/me/transitiveMemberOf` endpoint). Requires `ENABLE_OAUTH_GROUP_MANAGEMENT` to be enabled. Unresolvable IDs are left unchanged as a fallback.
- Persistence: This environment variable is a `PersistentConfig` variable.
